...
#define ONE_SHOT_LOG(n, tid)       \
  if (n >= RDMA_LOG_LEVEL) \
  OneShotThreadLogger((char*)__FILE__, __LINE__, n, tid).stream()

// Use the logical thread ID
class OneShotThreadLogger {
 public:
  OneShotThreadLogger(const char* file, int line, int level, t_id_t tid) : level_(level), tid_(tid) {
...
#define TLOG_NO_LINE_NUM(n, tid)       \
  if (n >= RDMA_LOG_LEVEL) \
  LogicalThreadLoggerNoLineNum((char*)__FILE__, __LINE__, n, tid).stream()

// Use the logical thread ID
class LogicalThreadLoggerNoLineNum {
 public:
  LogicalThreadLoggerNoLineNum(const char* file, int line, int level, t_id_t tid) : level_(level), tid_(tid) {
    if (level_ < RDMA_LOG_LEVEL)
      return;
  }

  ~LogicalThreadLoggerNoLineNum() {
    if (level_ >= RDMA_LOG_LEVEL) {
      std::ofstream fout;
      std::string log_file_name = "./" + std::to_string(tid_) + "_log.txt";
      fout.open(log_file_name, std::ios::app);
